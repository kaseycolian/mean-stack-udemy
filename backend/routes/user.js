...
router.post("/login", (req, res, next) => {
  ...
  User.findOne({ email: req.body.email }).then(user => {
    ...
    .then(result => {
      if (!result) { // if false(doesn't match)
        return res.status(401).json({
          message: "Password auth failed"
        });
      }
      const token = jwt.sign({ email: fetchedUser.email, userId: fetchedUser._id }, // creates new token w/ desired input data ie: email & user ID
        'secret_this_should_be_a_long_string', // this is required & used to validate hashes
